...
*  Odoo Await
*
* A simple way to communicate with Odoo API featuring Promises.
...
class OdooAwait {
   ...
   * @param opts - configure connection to Odoo
   */
  constructor (opts) {
    ...
    if (!this.port) {
      this.port = url.parse(options.baseUrl).port;
    }
   ...
   * Create a record.
   * @param model - e.g. 'res.partner'
   * @param params - Initial data to include in record eg: { name: 'some name', email: 'email@example.com }
   * @param [externalId] - this is a special parameter. Supplying it will
   ...
   * @param [moduleName] - custom module name for external ID (optional).
   ...
   * @param model - e.g. 'res.partner
   * @param recordId - a record ID integer or an array of ID's e.g. `45` or `[45, 15748, 347]`
   * @param fields - optional list of fields to return instead of all fields
   */
   ...
   * Update record
   * @param model - e.g. 'res.partner'
   * @param recordId - e.g. 45 or [45, 271]. If you provide
   * an array, the same value(s) are updated in every record.
   * @param params - Whatever fields you want to update e.g.
   ...
   * Delete a record
   * @param model - e.g. 'res.partner'
   * @param recordId -  e.g. 45 or [45, 667, 857]
   ...
   * @param model e.g. 'res.partner'
   * @param domain What to match in search e.g. country_id: 'United States'.
   * provide empty object to return all records. Leave out or provide an empty
   * object to return all records.
   * @param fields - fields to return in response e.g. ['name', 'state_id']
   * @param opts - options e.g. { offset: 100, limit: 5, order: 'category, birthday desc' }
   ...
   * @param model - e.g. 'res.partner'
   * @param domain - What to match / filter in search e.g.
   ...
   * @param model -  e.g. res.partner (required)
   * @param attributes - which attributes you want returned for each field.
   ...
   * @param model - model name of record the external ID is being created for
   * @param recordId - the Odoo record ID of the record
   * @param externalId - the unique identifier
   * @param moduleName - a module name is always appended to the beginning of an
   ...
   * @param externalId
   * @param fields - optional array of fields to return. If left blank
   */
   ...
   * @param externalId - the records external identifier
